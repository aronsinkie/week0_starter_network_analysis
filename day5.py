...
def selectHashTag():
    ...
    if hashTags:
        df = df[df['hashtags'].apply(lambda x: any(tag in x for tag in hashTags))]
        st.write(df)
...
def selectLocAndAuth():
    ...
    if location and not lang:
        df = df[df['place_coordinate'].isin(location)]
        st.write(df)
    elif lang and not location:
        df = df[df['language'].isin(lang)]
        st.write(df)
    elif lang and location:
        df = df[df['place_coordinate'].isin(location) & df['language'].isin(lang)]
        st.write(df)
    else:
        st.write(df)
...
st.title("slack Message  Display")
#selectHashTag()
