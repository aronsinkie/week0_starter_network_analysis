    ...
    def slack_parser(self,path_channel): 
        ...
        for json_file in glob.glob(f"{path_channel}/**/*.json"):
         with open(json_file, 'r', encoding="utf8") as slack_data:
             content = slack_data.read()  # Read the content of the file
             combined.append(content)  # Append the content to the list

        dflist = []
        for slack_content in combined: 
            data = json.loads(slack_content)  # Parse the JSON content
...
def main():
    ...
    # Prompt the user to enter the path to the Slack data folder
    path = r"C:\Users\CIAD\Music\week0_starter_network_analysis\anonymized"
    
    # Create an instance of SlackDataLoader with the specified path
    slack_data_loader = SlackDataLoader(path)
    
    # Access the channels and users
    channels = slack_data_loader.channels
    users = slack_data_loader.users
    ath = slack_data_loader.path
    part = slack_data_loader.get_community_participation()
    react=slack_data_loader.parse_slack_reaction( channels)
    print("ract",react)
    print("aro:",part)
    print("path",ath)
